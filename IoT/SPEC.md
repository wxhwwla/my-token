...
**文件管理状态（2026-08 全部完成）**：文件列表（动态模板）✅ / 上传（multipart）✅ / 删除（urlencoded + 白名单）✅ / 模块化（web_server 路由 + file_handling 文件操作 + protected_files_names 白名单）✅ / SD 卡挂载 ✅
...
**上传实现要点**：
- 解析 `multipart/form-data` 格式的 POST body（已实现，curl 实测通过）
- 文件名支持中英文
...
**删除实现（用户决策 2026-08）**：
- **交互方式**：输入框 + 表单（页面已有文件名，CV 粘贴后点删除）——不用每行删除链接（避免 URL 百分号编码问题，实现更简单）
- 提交格式：`POST /delete` + `application/x-www-form-urlencoded` body（`file=文件名`）——与上传的 multipart 区分：传文本键值对用 urlencoded 即可
- **批量删除扩展（规划）**：文件名空格分隔（`file=a.py b.py`），`split(b" ")` 循环——`delete_file` 函数签名已为此预留
- 白名单检查在 `delete_file` 内：受保护文件拒绝删除（返回"受保护"提示）
- 删除逻辑在 file_handling.py（与 get_file 同模块，保持模块化）
...
- **现状**：手动 `mpremote cp` 单文件上传——文件已 8 个，手动烦
- **目标（用户需求 2026-08）**：一键部署——上传全部 .py/.html → 软重启 → 自动恢复 → HTTP 验证
- **依赖**：重启后自动恢复需要**自启动**（boot.py 自动连 WiFi + 启动服务器）——否则重启后需手动 import
- **实施顺序**：① boot.py 自启动（并入 wifi_connect + web_server 启动）→ ② deploy.py（电脑端：收集 TX 全部 .py/.html → mpremote 上传 → soft reset → 等待 → HTTP 验证 /filemanager 200）
- **风险**：boot.py 改动有自举风险（本地有副本，坏可回传；§15 自动回滚后续补）
- **最终形态**：走 WiFi 的 /upload 接口 + 重启按钮——不插 USB 全流程（重启按钮随自启动一起做）

### 15. 自举防护（救生机制，规划中）

**问题（用户发现）**：坏代码上传（语法错误/import 失败）→ 服务器启动失败 → 修复需要上传 → 上传靠服务器 → **死锁，只能插 USB**。这是远程设备的经典"自举问题"。

**方案（三道防线，与自启动一起实施）**：
1. **上传语法检查**：上传 .py 时用 `compile()` 验证语法，语法错误直接拒绝（挡 90% 的"坏"——最常见的是语法错）
2. **自动备份**：上传覆盖前把旧文件存为 `.bak`（核心系统文件：boot.py、web_server.py、config.py 等）
3. **启动回滚**：boot.py 尝试启动主服务器，失败（异常）→ 自动用 `.bak` 恢复旧版 → `machine.reset()` 重启

**阶段**：与"自启动"（boot.py 自动连接 WiFi + 启动服务器 + 网页重启按钮）一起实施——基础设施文件管理完成后。

**不在范围**：救生服务器（备用最小服务器）——过度设计，等内网穿透/生产形态再评估。
...
| **文件名白名单保护** | **硬件层** | ✅ 已完成（boot.py 拒绝删除，curl 实测） |
| **非白名单文件删除** | **硬件层** | ✅ 已完成（upload_test.py 删除成功，curl 实测） |
| **删除不存在文件** | **硬件层** | ✅ 已完成（返回"文件不存在"，不崩溃） |
| **上传语法检查** | **硬件层** | 上传语法错误的 .py，验证被拒绝且服务器存活（规划中，自举防护） |
| **文件列表正确性** | **硬件层** | ✅ 已完成（/filemanager 实测：文件列表 + 大小 + sd 挂载点显示正常） |
| **上传坏代码拒收** | **硬件层** | 上传语法错误的 .py，验证被拒绝且服务器存活（规划中） |
| **启动回滚** | **硬件层** | 上传运行时错误的代码，验证 boot.py 自动恢复 .bak 并重启（规划中） |
